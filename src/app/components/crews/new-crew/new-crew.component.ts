...
export class NewCrewComponent implements OnInit {
  ...
  validCrewsForm(): boolean {
    ...
    if (!this.crewsDataForm.name) {
      this.auxService.AlertWarning('Data invalid.', 'Error: name is required.');
      return false;
    }
    if (!this.crewsDataForm.shiftHours) {
      this.auxService.AlertWarning(
        'Data invalid.',
        'Error: shiftHours is required.'
      );
      return false;
    }
    if (!this.crewsDataForm.patternDays) {
      this.auxService.AlertWarning(
        'Data invalid.',
        'Error: Days Until Pattern Repeats is required.'
      );
      return false;
    }
    if (!this.crewsDataForm.rotationDay) {
      this.auxService.AlertWarning(
        'Data invalid.',
        'Error: 1st Day of Rotation is required.'
      );
      return false;
    }
    if (!this.crewsDataForm.crewNumber) {
      this.auxService.AlertWarning(
        'Data invalid.',
        'Error: Number of Crews is required.'
      );
      return false;
    }
  ...
  validCrews(): boolean {
    for (let i = 0; i < this.crewData.length; i++) {
      ...
      if (!crew.team || crew.team <= 0) {
        this.auxService.AlertWarning(
          'Data invalid.',
          `Error in Crews at index ${
            i + 1
          }: "team" is required and must be greater than 0.`
        );
        return false;
      }
    ...
    for (let i = 0; i < this.shiftData.length; i++) {
      ...
      if (
        ...
        this.auxService.AlertWarning(
          ...
          `Error in Shifts at index ${
            i + 1
          }: "team" is required and must be a non-empty string.`
        );
        return false;
      }
    }
  ...
  fillTeamSelect(): void {
    if (!this.crewDataInput || this.crewDataInput.length === 0) {
      this.auxService.AlertWarning(
        'No data',
        'The crewDataInput array is empty.'
      );
      return;
    }
    ...
    this.auxService.AlertSuccess(
      'Success',
      'Teams have been assigned in order.'
    );
  }
