...
@Component({
  ...
  templateUrl: './centerlines.component.html',
  styleUrl: './centerlines.component.css'
})
export class CenterlinesComponent implements OnInit {
  machines: any[] = [];
  categories: any[] = [];
  centerlines: any[] = [];
  searchValue: string = '';
  ...
  drop(event: CdkDragDrop<DataItem[]>): void {
    moveItemInArray(this.centerlines, event.previousIndex, event.currentIndex);
  }

  Continue() {
    this.auxService.ventanaCargando();
    this.arrangeService
      .arrange('Arrange-centerlines', this.centerlines)
      .subscribe({
        next: async (data: any) => {
          this.auxService.cerrarVentanaCargando();
          if (data.success) {
            return this.auxService.AlertSuccess('Centerlines arranged successfully.', '');
          } else {
