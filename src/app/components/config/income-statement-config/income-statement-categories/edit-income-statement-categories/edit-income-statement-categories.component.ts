...
@Component({
  selector: 'app-edit-income-statement-categories',
  standalone: true,
  imports: [NzInputModule, NzIconModule, CommonModule, ReactiveFormsModule, MatDialogModule, SharedModule, NzFormModule],
  templateUrl: './edit-income-statement-categories.component.html',
  styleUrl: './edit-income-statement-categories.component.css'
})
export class EditIncomeStatementCategoriesComponent {
  idCategory: number;
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<EditIncomeStatementCategoriesComponent>
  ) {
  ...
  cargarDetalles() {
    ...
    this.configService.ObtenerDetailERTipoCategoria(this.idCategory).subscribe({
      next: (data) => {
  ...
  guardarCambios() {
    if (this.clientForm.valid) {
      ...
      this.configService.ActualizarERTipoCategoria(this.idCategory, this.clientForm.value).subscribe({
        next: (data) => {
          if (data.success) {
